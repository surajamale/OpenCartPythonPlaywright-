...
class LocatorWrapper:
    """
    Wrap a Playwright Locator so that calling wait_for(...) returns the Locator
    itself (useful for code that expects a Locator after waiting).
    """
    ...
    def __getattr__(self, item):
        # forward other attribute/method accesses to the underlying locator
...
class RegistrationPage:
    ...
    def get_confirmation_msg(self):
        """
        Return a wrapper around the confirmation message locator.
        Calling .wait_for(...) on the returned object will wait and then
        return a Locator suitable for expect(locator).to_have_text(...).
        """
        return LocatorWrapper(self.msg_confirmation)
